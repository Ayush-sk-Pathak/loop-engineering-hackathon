...
export class DemoStore {
  ...
  private readonly tables: { state: string; events: string; incidents: string };
  private runStartedAt: string | null = null;

  constructor(
    path = process.env.SQLITE_PATH ?? "data/continuim.db",
    ...
    this.tables = {
      state: `demo_state${suffix}`,
      events: `decision_events${suffix}`,
      incidents: `incidents${suffix}`,
    };
    this.database.exec(`
      PRAGMA journal_mode = WAL;
      CREATE TABLE IF NOT EXISTS ${this.tables.state} (
  ...
  reset(scenarioId?: ScenarioId, hard = false): DemoState {
    ...
    this.database.exec(`DELETE FROM ${this.tables.events}`);
    if (hard) this.database.exec(`DELETE FROM ${this.tables.incidents}`);
  ...
  read(): DemoState | undefined {
    const row = this.database.prepare(`SELECT payload FROM ${this.tables.state} WHERE id = 1`).get() as
      | { payload: string }
      | undefined;
  ...
  setClientIncident(clientId: "meridian" | "northwind", nodeId: string, faultType: string): DemoState {
    return this.update((state) => ({
      ...state,
      clientIncident: { clientId, nodeId, faultType, detectedAt: new Date().toISOString() },
    }));
  }
  ...
  appendEvent(event: DecisionEvent): DemoState {
    this.database.prepare(
      `INSERT INTO ${this.tables.events} (id, correlation_id, phase, payload, occurred_at) VALUES (?, ?, ?, ?, ?)`,
  ...
  private recordIncident(input: {
    ...
    this.database.prepare(
      `INSERT INTO ${this.tables.incidents} (id, scenario_id, sku, resolution_ms, ordered_vendor_id, payload, resolved_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      incident.id,
      incident.scenarioId,
      incident.sku,
  ...
  private learningSummary(scenarioId: ScenarioId): DemoState["learning"] {
    const count = this.database
      .prepare(`SELECT COUNT(*) AS count FROM ${this.tables.incidents} WHERE scenario_id = ?`)
      .get(scenarioId) as { count: number };
    const last = this.database
      .prepare(`SELECT resolution_ms FROM ${this.tables.incidents} WHERE scenario_id = ? ORDER BY rowid DESC LIMIT 1`)
      .get(scenarioId) as { resolution_ms: number } | undefined;
    const proven = this.database
      .prepare(`SELECT DISTINCT ordered_vendor_id AS id FROM ${this.tables.incidents} WHERE scenario_id = ? AND ordered_vendor_id IS NOT NULL`)
  ...
  private write(state: DemoState): void {
    this.database
      .prepare(`INSERT INTO ${this.tables.state} (id, payload) VALUES (1, ?) ON CONFLICT(id) DO UPDATE SET payload = excluded.payload`)
